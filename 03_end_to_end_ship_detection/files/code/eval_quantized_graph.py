...
def graph_eval(input_graph_def, input_node, output_node, model_type):
    ...
    ax[2].set_title('Prediction, IoU: ' + str(IoU))
    plt.savefig("../rpt/unet_model_progress_14_m_" + str(model_type) + "_" + str(WIDTH) + "x" + str(HEIGHT) + ".png")
    plt.show()

    n_samples = np.min(4,cfg.BATCH_SIZE)
    ...
    for idx,jdx in enumerate (random.sample(range(0,y_pred.shape[0]), n_samples)):
        ...
        axs[0,2].set_title('Prediction, IoU: ' + str(IoU))

        plt.xticks([])
        plt.yticks([])
        plt.savefig("../rpt/unet_model_progress_15_m_" + str(model_type) + "_" + str(WIDTH) + "x" + str(HEIGHT) + ".png")
        plt.show()

    mIoU = eus.IoU_all (y_test,y_pred)
    print ("IoU for the test dataset: ", mIoU)
